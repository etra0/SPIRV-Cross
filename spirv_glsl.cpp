...
static bool check_forward_limit_exceeded(uint32_t op0, const char *op)
{
	static uint32_t forward_count = 0;
	static const char *last_op = nullptr;
	static uint32_t last_op0 = 0;

	if (last_op0 == op0 && last_op == op)
	{
		forward_count++;
	}
	else
                                       ...
                                       const char *op, SPIRType::BaseType input_type, bool skip_cast_if_equal_type)
{
	...
	bool forward_limit_exceeded = check_forward_limit_exceeded(op0, op);
